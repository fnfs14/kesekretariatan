...
class Administrator extends CI_Controller {
    ...
    public function feedback()
    {
        ...
        foreach ($setum as $key1) {
            $this->db->query("INSERT INTO notadinas.feedback_surat_masuk (id_surat_masuk,pengirim,pesan_feedback,created_at,waktu,penerima) VALUES ($id,$pengirim,'$pesan','NOW()','$time','".$key1->id."')");
        }
        ...
        foreach ($disp as $key) {
            if($key->penerima_disposisi != $pengirim){
    ...
    public function feedbacknotas()
    {
        ...
        foreach ($setum as $key2) {
            $this->db->query("INSERT INTO notadinas.feedback_nota_dinas (id_nota_dinas,pengirim,pesan_feedback,created_at,waktu,penerima) VALUES ($id,$pengirim,'$pesan','$date','$time','".$key2->id."')");
        }
        ...
        if($ygbuat2->satuan==""){
			...
			if($ygbuat2->satuan!=6 and $ygbuat2->tingkatan){
	...
	public function master_surat_masuk(){
        ...
        if ($aksi == "cari") {
            $a['data'] = $this->db->query("SELECT * FROM master_jenis_surat_masuk WHERE nama_jenis_surat ILIKE '%$cari%' ORDER BY id_jenis_surat_masuk DESC")->result();
            $a['page'] = "l_master_surat_masuk";
        }else if($aksi=="add"){
            $a['page']	= "f_master_surat_masuk";
        }else if($aksi=="m_surat_masuk"){

            $a['data']	= $this->db->query("SELECT * FROM notadinas.master_jenis_surat_masuk")->result();
            $a['page']	= "l_master_surat_masuk";
        }else if($aksi == "save"){
            $this->db->query("INSERT INTO notadinas.master_jenis_surat_masuk VALUES (DEFAULT,'".$jsk."','".$isk."')");
            $a['data']	= $this->db->query("SELECT * FROM notadinas.master_jenis_surat_masuk")->result();
            redirect('administrator/master_surat_masuk/m_surat_masuk');
        } else if($aksi=="edit"){
            $a['page']	= "u_master_surat_masuk";
            $a['data_surat_masuk'] = $this->db->query("SELECT * FROM notadinas.master_jenis_surat_masuk where id_jenis_surat_masuk = '$id_surat_masuk'")->result();
        } else if($aksi=="update"){
            $this->db->query("UPDATE notadinas.master_jenis_surat_masuk SET isi_jenis_surat = '".$isk."', nama_jenis_surat = '".$jsk."' where id_jenis_surat_masuk = '$id_surat_masuk'");
            $a['data']	= $this->db->query("SELECT * FROM notadinas.master_jenis_surat_masuk")->result();
            redirect('administrator/master_surat_masuk/m_surat_masuk');
        }else if($aksi=="delete"){
            $this->db->query("DELETE FROM notadinas.master_jenis_surat_masuk where id_jenis_surat_masuk = '$id_surat_masuk'");
            $a['data']	= $this->db->query("SELECT * FROM notadinas.master_jenis_surat_masuk")->result();
            redirect('administrator/master_surat_masuk/m_surat_masuk');
        }
