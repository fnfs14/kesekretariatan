		...
		<tr>
			...
			if($b->status_surat_masuk == 1 or $b->status_surat_masuk == 2){
					...
					if($this->session->userdata('admin_satuan') == 6 || $this->session->userdata('admin_jabatan') == 1 || $this->session->userdata('admin_jabatan') == 28){
						...
						$_waktu = "";
						if(isset($_tmp_query->waktu) and $_tmp_query->waktu!=""){
			...
			if($b->status_surat_masuk==1){ // setum create
				if($satuan==6){
			?>
			...
			<?php }elseif($satuan==6){ // dikirim setum?>
			...
			<?php }else if($tingkatan==1 and isset($arrayDisp[$jabatan])){ // kadis mendisposisi
					...
					if($kadisDisp!=NULL and $kadisDisp->disposisi == 'yes'){ ?>						
					...
					<?php } ?>
			<?php }else if($satuan==6){ // info di setum
					$funZ = "edt";
