...
ORDER BY updated_at DESC")->result();
		...
		foreach($bell as $bl){ //qqq
			...
			if($bl->tablenya=="masuk" and !isset($notif['masuk'][$bl->idnya])){
				...
				}else if($tingkatan==2 and $bl->status_surat_keluar==4 and $bl->ka_waka_setum=="subdis" and $user_subjabatan!=NULL and $bl->id_jabatan==$user_subjabatan->id and $bl->opened==4){
			...
			}else if($bl->tablenya=="keluar" and !isset($notif['keluar'][$bl->idnya])){
	...
	function getSelectedJabatan($z=NULL){
		if($z!=NULL){
			$z = " AND id != $z";
		}
		return $this->db->query("SELECT * FROM notadinas.master_jabatan WHERE id = 1 OR id = 28 OR (urutan_view IS NOT NULL AND urutan_view != 0$z) ORDER BY urutan_view ASC")->result();
	}
    ...
    function cetak_surat_masuk($idu)//ubah mei surmas5
    {
        ...
        $a['jabatan'] = $this->db->query("
		SELECT notadinas.master_jabatan.*
		FROM notadinas.surat_masuk
		INNER JOIN notadinas.master_jabatan
		...
		if(
			...
			$kadisp['alamat_aksi_sub'] = $this->db->query("
				SELECT 
					a.id AS id,
					b.id_jabatan as id_jab,
					b.urut_subjabatan as urut_subjab,
					b.nama_subjabatan
...
//sisexy
    public function surat_masuk()
    {
        ...
        if ($mau_ke == "save_task") {
			...
			$a['jabatan'] = $this->getSelectedJabatan();
            $a['jenis_surat'] = $this->db->query('SELECT * FROM notadinas.master_surat_masuk')->result();//ubah surat masuk mei
            $a['aksi'] = $this->db->query("SELECT * FROM notadinas.master_aksi ORDER BY urutan ASC")->result();
            $a['query1'] = $query = $this->eoffice->query("SELECT * FROM fo_objects WHERE object_type_id = 1 AND trashed_on = '0000-00-00 00:00:00' AND trashed_by_id = 0")->result();
            $a['query2'] = $query = $this->eoffice->query("SELECT * FROM fo_objects WHERE object_type_id = 5 AND trashed_on = '0000-00-00 00:00:00' AND trashed_by_id = 0")->result();
			...
			$_tingkatan = $this->db->query('SELECT tingkatan,satuan FROM notadinas.master_jabatan WHERE id = ' . $cc->kepada)->row();
            $jabatan_push_notif = "";
			if($cc->status_surat_masuk == 30){
				$this->db->query("UPDATE notadinas.surat_masuk SET status_surat_masuk = 3, opened = 3, updated_at = '$upd_date' WHERE id = '$idu'");
				$jabatan_push_notif .= "28,1,";
				if($_tingkatan->tingkatan==1){
					$jabatan_push_notif .= "$kepada,";
				}else if($_tingkatan->tingkatan==2){
					$jabatan_push_notif .= "$kepada,";
					$qKadis = $this->db->query("SELECT * FROM notadinas.master_jabatan WHERE satuan = $_tingkatan->satuan AND tingkatan = 1")->result();
					foreach($qKadis as $qK){
						$jabatan_push_notif .= "$qK->id,";
					}
				}
			}else if($cc->status_surat_masuk == 40){
				$this->db->query("UPDATE notadinas.surat_masuk SET status_surat_masuk = 4, opened = 4, updated_at = '$upd_date' WHERE id = '$idu'");
				$jabatan_push_notif .= "28,1,";
				if($_tingkatan->tingkatan==1){
					$jabatan_push_notif .= "$kepada,";
				}else if($_tingkatan->tingkatan==2){
            ...
            $jabatan = $this->getSelectedJabatan();
            ...
            foreach ($jabatan as $val) {
                ...
                if (isset($data['action' . $val->id])) {
					...
					if($checkTingkatan==2){
						$cekSatuan = $this->db->query("SELECT satuan FROM notadinas.master_jabatan WHERE id = $jab")->row()->satuan;
						$getJabatan = $this->db->query("SELECT id FROM notadinas.master_jabatan WHERE satuan = $cekSatuan AND tingkatan = 1")->result();
						foreach($getJabatan as $gJ){
							if(!isset($data['jbtn' . $gJ->id]) and $data['jbtn' . $gJ->id]==NULL and $data['jbtn' . $gJ->id]==""){
								$this->db->query("INSERT INTO notadinas.disposisi_surat_masuk ($_xCol, id_log_proses, jenis, status, aksi, keterangan, id_surat_masuk, disposisi) VALUES ($gJ->id, $idn, 'INFORMASI', 1, 0, '$kets', $ind, '');");
							}
						}
     ...
     public function nota_dinas()
    {
        ...
        if ($mau_ke == "cetak_nota_dinas") {
           ...
           $a['status_kadis'] = $this->db->query("SELECT *,notadinas.master_jabatan.nama_jabatan AS nama_kadis, notadinas.nota_dinas.id AS id_nota FROM notadinas.nota_dinas INNER JOIN notadinas.master_jabatan ON notadinas.nota_dinas.kepada = notadinas.master_jabatan.id WHERE notadinas.nota_dinas.status_notadinas = '4'")->result();
			...
			$a['data'] = $this->getSelectedJabatan();
            $a['user_n'] = $a['data'];
           ...
           $data = addslashes($this->input->post('data'));
			...
			$a['data'] = $this->getSelectedJabatan($this->session->userdata("admin_jabatan"));
            $a['user_n'] = $a['data'];
            //$a['kegiatan']    = $this->db->query("SELECT * FROM notadinas.kegiatan_dinas")->result();
            $a['page'] = "f_nota_dinas";
        } else if ($mau_ke == "edt") {
...
//prima
    ...
    public function cetak_no_lampiran_sm($a, $b, $c, $d)//ubah mei surmas8
    {
        $asd = ['a' => $a . "-" . $b . "-" . $c . "-" . $d];
        $this->load->view('admin/notadinas/cetak_nolampiran', $asd);
    }
